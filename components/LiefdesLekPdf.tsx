...
import { STELLINGEN, STRATEGIEEN, OVERTUIGINGEN, scoreband, type Slider2 } from './LiefdesLek';
...
function LiefdesLekDocument({ d1, gekozenStrategieen, aangevinktOv, slidersOv, kernOvertuigingen, aangevinktKern, slidersKern, analyse }: Props) {
  ...
  return (
    <Document title="Flauwekul Filter: LiefdesLek" author="Energieke Lieke">
      <Page size="A4" style={s.pagina}>
        <PdfHeader titel="Flauwekul Filter: LiefdesLek" datum={datum} />
        ...
        <View style={s.body}>
          ...
          {/* DEEL 2 */}
          <PdfSectieKop titel="DEEL 2: LIEFDESSTRATEGIEEN" />
        ...
        <PdfFooter titel="Flauwekul Filter: LiefdesLek" />
      </Page>
    </Document>
  );
}

function LiefdesLekPdfKnopInner(props: Props) {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { PDFDownloadLink } = require('@react-pdf/renderer');
  return (
    <PDFDownloadLink document={<LiefdesLekDocument {...props} />} fileName="flauwekul-filter-liefdeslek.pdf">
...
export const LiefdesLekPdfKnop = dynamic(
  () => Promise.resolve(LiefdesLekPdfKnopInner),
